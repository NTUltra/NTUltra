...
function EnemyHealthAdjustments(){
	if instance_exists(Player)
	{
		...
		maxhealth *= 1 + max(0,(Player.loops * 0.04));//vanilla 0.05
		if Player.loops > 0
			maxhealth += 1;
		maxhealth += Player.loops * 0.75;
		if scrIsHardMode()
			maxhealth *= 1.1;
		if scrIsCrown(10)
		...
		if Player.skill_got[33] = 1//GLASS ARM CANNON
		{
			...
			if Player.race == 25
				...
				my_health *= 0.67 + clamp(ogmaxhealth*0.00006,0,0.3);//should really be 65% but micro balance
		}
		/*
