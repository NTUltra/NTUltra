...
*/
	...
	ultra_tips[28] = "<3 bullets"
		...
		else
		{
			...
			if isOtherCharacter
				ultra_text[29] = "ONLY HIGH TIER WEAPON DROPS#EVERY ENEMY HAS A +1% CHANCE#TO DROP A WEAPON#INCREASE FIRERATE BASED ON WEAPON TIER"
			else
				ultra_text[29] = "ONLY HIGH TIER WEAPON DROPS#EVERY ENEMY HAS A +1% CHANCE#TO DROP A WEAPON#INCREASE FIRERATE BASED ON WEAPON TIER##AUTO EAT WEAPONS LEFT IN THE LEVEL"
	...
	*/
	...
	if altOverride || (instance_exists(Player) &&  Player.race == 19 && (Player.ultimategamble==true || Player.unlockAlternativeUltras))
	{
		...
		&& instance_exists(UltraIcon)))))
		{
		    ...
		    ultra_text[77] = "GO TO YV'S CRIB#YOUR SHOP BECOMES A WEAPON WHEEL#SELECT A SLOT TO STORE#YOUR PRIMARY WEAPON#WEAPONS CAN BE TAKEN OUT AT ANY TIME#THRONE BUTT DOUBLES INVENTORY SIZE##AT THE START OF EACH LOOP#OVERHEAL BASED ON TOTAL#WEAPON TIER#IN YOUR INVENTORY"
	...
	ultra_text[92] = "NORMAL CONTROLS##WHILE IN COMBAT#SLOWLY CONSUME GAS TO HEAL"
	ultra_tips[92] = "you can gas while moving"
	    ...
	    if instance_exists(Player)
	    {
		    if  Player.race == 23 && (Player.altUltra || Player.unlockAlternativeUltras || (Player.wep_type[Player.wep] == 3 && Player.wep_type[Player.bwep] == 3 && instance_exists(UltraIcon))){
				...
				ultra_text[92] = "NORMAL CONTROLS#EMIT SPLINTERS INSTEAD OF GAS#ALL SPLINTERS ARE GASEOUS"
	    ...
	    ultra_text[93] = "YOU AND YOUR PROJECTILES#PHASE THROUGH YOUR WALLS#BIGGER TERRAFORM"
	    ultra_tips[93] = "get to cover!"
    
	    ultra_name[94] = "ICE COLD"
	    ultra_text[94] = "COOL EVERYTHING NEAR YOU#STRONGER & HOMING DEBRIS#DEBRIS FREEZES ENEMIES##YOUR WALLS CREATE#MORE DEBRIS WHEN DESTROYED#BIGGER TERRAFORM"
