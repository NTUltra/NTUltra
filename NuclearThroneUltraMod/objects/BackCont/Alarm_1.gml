...
if  (( (Player.loops >= 1 and instance_number(Grunt) < 3+Player.loops and Player.area != 100) || Player.crown=10) && instance_number(enemy) - instance_number(IDPDVan) > 3)
{
repeat(min(9,Player.loops))    
instance_create(Player.x,Player.y,IDPDSpawn)        
if Player.crown==10//crown of difficulty
{
	repeat(min(1+Player.loops,4)) 
	instance_create(Player.x,Player.y,IDPDSpawn)
}
if Player.crown==16//crown of popo
{
	repeat(min(2+Player.loops,9)) 
	instance_create(Player.x,Player.y,IDPDSpawn)
}
}
